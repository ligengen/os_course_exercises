...
2. 不同特权级有什么区别？
* 特权指令只可以在ring 0中使用，同一条指令在不同的特权级下的作用和能够访问到的区域也是不同的

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
* CPL(current priviledge level)是当前进程的权限级别，是执行的代码所在段的特权级，存在CS寄存器的低两位。DPL(Descriptor priviledge level)存储在段描述符中，规定了访问该段的权限级别。RPL(request priviledge level)是进程对段访问的请求权限。
* 访问门的时候，CPL <= DPL[门] & CPL >= DPL[段]
* 访问段的时候，max(CPL, RPL) <= DPL[段]
* 中断响应是访问中断门，所以是 CPL <= DPL[门] & CPL >= DPL[段]
* 函数调用和指令执行都是高优先级的可以访问低优先级的数据段
...
* 访问数据时，需要检查当前特权级是否高于要访问的数据段
* 控制转移的时候，只允许低特权级调用高特权级
* 发生中断和异常时，也需要判定特权级
...
* ring3会产生用户态的栈转换到特权态的栈，而且需要把用户态的栈的ss和esp保存到内核态的栈中

4. 如何利用int和iret指令完成不同特权级的切换？
* 人工构造所需的栈结构，用int和iret进行切换（？不懂。。这是在问啥）

5. TSS和Task Register的作用是什么？
* TSS保存了任务的有关信息，在任务内变换特权级和任务切换需要用到TSS
* TR寄存器用于设置TSS。TR的选择子在索引中查到TSS描述符之后，将描述符加载到TR寄存器的隐藏部分，保存了TSS的相关设置
...
* 逻辑地址通过段机制转换为线性地址，再通过页机制转换为物理地址
...
* [x] 中断处理过程中
* [ ] ALU计算过程中
* 因为ALU计算所需的数据已经在CPU中，不需要检查访问权限了

请描述ucore OS建立页机制的准备工作包括哪些步骤？ \(w4l1\)
* 初始化GDT表，完成段机制，用lgdt gdtdesc指令载入即可
* 对物理内存进行探测并对空闲的物理内存进行管理
* 给CR0寄存器最后一位置1，进入保护模式，之后跳转到protseg部分，将数据段选择子赋给一些通用寄存器，再将ebp清零，指向底层调用，将esp指向0x7c00，最后调用bootmain函数。
